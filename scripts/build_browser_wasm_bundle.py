...
def _build_variant(
    *,
    ...
    for path, label in (
        ...
        (certificate_path, "safety certificate"),
    ...
    if (
        ...
        raise ValueError(
            f"{runtime_variant} safety certificate has invalid thread_count"
        )
...
def validate_existing_bundle(bundle: Path, source_package: Path) -> Mapping[str, Any]:
    ...
    certificate = _require_mapping(
        variant.get("safety_certificate"),
...
def main() -> None:
    ...
    missing = [
        ...
        for name, value in (
            ...
            ("--single-certificate", arguments.single_certificate),
    ...
    assert arguments.single_certificate is not None
    assert arguments.output is not None
    manifest = build_bundle(
        single_wasm=arguments.single_wasm.resolve(),
        single_module_js=arguments.single_module_js.resolve(),
        single_certificate_path=arguments.single_certificate.resolve(),
